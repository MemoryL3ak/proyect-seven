...
La tercera versión (3.0) sumó la **puesta en marcha de la aplicación móvil nativa** y su operación cotidiana: **inicio de sesión móvil por código** con recuperación por correo, **notificaciones push** con **bandeja de notificaciones (campana)**, **panel administrativo de envío de push**, **gestión de permisos del dispositivo** y **seguimiento GPS nativo del conductor en segundo plano** (Parte XIII).

Esta cuarta versión (4.0) documenta las funcionalidades liberadas entre julio y agosto de 2026, en la nueva **Parte XIV**: **sesión única por dispositivo** en los portales, **eliminación de cuenta a pedido del propio usuario** (con reactivación desde administración), **Transfer In / Transfer Out con monitor de vuelos** y mapa interactivo del recorrido, **Monitoreo VIP** con tracking permanente de ubicación, **Panel Financiero de transporte** (ingreso, costo y margen), **ficha de salud del participante** con descarga de credencial completa en PDF, y la operación del **Portal Partner** para el canje de beneficios en comercios. Además se actualiza el catálogo de portales (Parte IX) con sus URLs vigentes.

El manual está organizado en **catorce partes** y **tres anexos**. Cada parte agrupa módulos relacionados y describe:
...
|--------|-----|--------------|
| Usuario (Atleta / Jefe de Delegación) | `/portal/user` | Participantes tipo TA; el jefe de delegación ve secciones adicionales |
| Solicitud de Vehículo (VIP) | `/portal/vehicle-request` | Autoridades y VIPs (el login por código en `/portal/user` redirige aquí automáticamente si el participante es VIP) |
| Conductor | `/portal/conductor` | Conductores de vehículos (propios y de proveedor) |
| Control de Acceso | `/portal/access-control` | Staff de acceso (participantes de proveedor tipo Staff) |
| Partner (Beneficios) | `/portal/partner` | Comercios aliados que canjean cupones (login con código + PIN) |

Todos comparten una identidad visual común: autenticación por **código personal de 6 caracteres** (últimos 6 del ID del participante) y opción de "Solicitar código de acceso". El portal Partner es la excepción: usa **código de comercio + PIN** definidos por el administrador.

📌 **Nota (v4.0):** las URLs históricas `/portal/vip`, `/portal/delegation` y `/portal/athlete` fueron consolidadas: hoy el participante ingresa siempre por `/portal/user` (o la app móvil) y el sistema resuelve el portal según su tipo de cliente.
...
# PARTE XIV — NOVEDADES OPERATIVAS (V4.0)

Esta parte documenta las funcionalidades liberadas entre julio y agosto de 2026. Complementan a los módulos ya descritos; cuando una novedad modifica el comportamiento de un capítulo anterior, se indica la referencia.

## 55. Sesión única por dispositivo en los portales

### 55.1 Qué es

Cada cuenta de portal (participante o conductor) puede tener **una sola sesión activa a la vez**. Si la cuenta ya está abierta en un dispositivo, un segundo dispositivo no puede iniciar sesión: el login se rechaza con el mensaje *"esta cuenta ya tiene una sesión activa en otro dispositivo"*.

### 55.2 Reglas

- **La sesión existente manda**: el segundo dispositivo no expulsa al primero; su login se rechaza.
- La sesión activa emite un **latido** periódico mientras el portal está abierto. Si la app se cierra sin cerrar sesión, el latido se corta y **a los ~2 minutos** otro dispositivo puede entrar.
- **Cerrar sesión libera la sesión de inmediato**, permitiendo entrar desde otro dispositivo sin espera.

🟡 **Atención:** si un usuario reporta que "no puede entrar porque hay otra sesión activa" y asegura no tener otro dispositivo abierto, basta esperar un par de minutos (latido vencido) o pedirle que cierre sesión en el dispositivo original.

## 56. Eliminación de cuenta y reactivación

### 56.1 Eliminar mi cuenta (lado usuario)

Todos los portales activos incluyen la opción **"Eliminar mi cuenta"**:

| Portal | Dónde está |
|--------|------------|
| Usuario | Pestaña **Cuenta**, bajo "Cerrar sesión" |
| Conductor | Final de la pestaña **Cuenta** |
| Solicitud de Vehículo (VIP) | Pestaña **Cuenta**, bajo "Cerrar sesión" |
| Control de Acceso | Junto al botón "Cerrar sesión" |
| Partner | Final de la pantalla del scanner |

Flujo:

1. Presionar **"Eliminar mi cuenta"**.
2. Confirmar en el diálogo de advertencia (la acción se describe como permanente).
3. La cuenta queda **desactivada**: se cierra la sesión en todos los dispositivos, el código de acceso deja de funcionar y ya no se reciben notificaciones push.

🔴 **Crítico:** la eliminación es un **soft delete**: los datos históricos (viajes, canjes, acreditación) se conservan para trazabilidad, pero la cuenta queda con estado `DELETED` y no puede volver a ingresar por sí sola.

### 56.2 Reactivar una cuenta (lado administración)

Una cuenta eliminada puede reactivarse **solo desde el módulo administrativo**:

| Tipo de cuenta | Dónde se reactiva |
|----------------|-------------------|
| Participante (atleta / VIP / jefe) | **Registro → Inscripción Participantes**: la tarjeta aparece con badge **ELIMINADA** y botón **"Reactivar"** |
| Conductor | **Users → Conductores**: fila con estado `DELETED` muestra el botón **"Reactivar"** |
| Participante de proveedor (staff / chofer externo) | **Registro → Proveedores**: badge **ELIMINADA** e ícono de reactivación junto a Editar |
| Partner de beneficios | **Beneficios → Administrar beneficios**: editar el partner y poner **"Activo: Sí"** |

Al reactivar, la cuenta recupera el **estado que tenía antes de la baja** y el usuario puede volver a ingresar con **su mismo código de siempre** (el código no cambia porque deriva del ID).

## 57. Transfer In / Transfer Out y Monitor de Vuelos

### 57.1 Qué es

Los viajes de recepción (**Transfer In**) y despedida (**Transfer Out**) quedan vinculados al **vuelo** del participante. El **Monitor de Vuelos** (Operación → Arribos & Salidas → Monitor de Vuelos) muestra las llegadas y salidas con su estado en tiempo real y una **línea de tiempo clickeable**.

### 57.2 Operar

1. Registrar el vuelo del participante (número, aerolínea, fecha) — ver capítulo 13.
2. Crear el viaje tipo **Transfer In** (o **Transfer Out**) asociando el vuelo: el sistema muestra la hora estimada actualizada del vuelo para planificar la recogida.
3. En el Monitor de Vuelos, **pinchar un vuelo en la línea de tiempo** abre directamente el editor del viaje asociado.
4. El detalle del viaje incluye un **mapa interactivo del recorrido** (ruta origen → destino con los puntos reportados por GPS).

🟢 **Tip:** ante un retraso de vuelo, el monitor lo refleja automáticamente (integración AviationStack); reprogramar el transfer desde el mismo editor.

### 57.3 Regla del cliente VIP

🟡 **Atención:** un cliente **VIP viaja solo**: sus viajes no arrastran delegación ni participantes adicionales. La plataforma lo aplica automáticamente al crear solicitudes.

## 58. Monitoreo VIP

### 58.1 Qué es

**Operación → Transporte → Monitoreo VIP** permite seguir en tiempo real la ubicación y agenda de los participantes VIP. Los VIP transmiten su ubicación de forma **permanente** mientras tienen la app abierta (no solo durante viajes).

### 58.2 Contenido

- **Mapa** con la posición actual de cada VIP.
- **Ficha completa** del VIP: datos personales, credencial, vuelos, hotel, viajes y premiaciones asignadas.
- **Timelines tipo tablero** con la agenda del día de cada VIP (viajes, premiaciones, actividades).

🟢 **Tip:** esta vista es la herramienta principal del equipo de protocolo durante ceremonias: permite anticipar atrasos de un VIP asignado a una premiación.

## 59. Panel Financiero de Transporte

**Operación → Transporte → Panel Financiero** consolida los indicadores económicos de la operación de transporte:

- **Ingreso** por viajes según tarifas aplicadas.
- **Costo** operativo (incluye kilometraje GPS real de los recorridos).
- **Margen** resultante, por día y acumulado.
- Desglose por proveedor, tipo de vehículo y tipo de viaje.

📌 El kilometraje GPS se calcula de los recorridos registrados, con caché para no saturar la API en consultas repetidas.

## 60. Ficha de salud y credencial PDF

### 60.1 Ficha de salud del participante

- El participante completa su **ficha de salud** desde el portal (Cuenta → "Ficha de salud"); el estado se muestra como *Completada* o *Pendiente*.
- El equipo de salud consulta las fichas desde **Operación → Salud**, con **detalle por participante**.

### 60.2 Credencial completa en PDF

- El botón de descarga de la credencial en los portales genera ahora el **PDF de la credencial completa** (mismo formato que la impresa, con QR, foto y datos del evento).
- Existe además una ruta de **descarga de credencial sin login** (`/credencial`) para casos operativos donde el participante no puede ingresar al portal.

## 61. Portal Partner — canje de beneficios

### 61.1 Acceso

1. Abrir `/portal/partner`.
2. Ingresar **código del comercio** (ej. `MCDO-001`) y **PIN** (definidos por el administrador en Beneficios → Administrar beneficios).
3. La sesión dura 12 horas; el botón **"Salir"** la cierra antes.

### 61.2 Operar el scanner

1. La pantalla principal muestra **estadísticas del comercio**: canjes de hoy, total acumulado, beneficios habilitados, pendientes por validar y stock restante.
2. Presionar **"Escanear"** y apuntar la cámara al QR del cupón del participante (o ingresar el **código manual** tipo `CPN-XXXXXX`).
3. El sistema valida y muestra el detalle del cupón; confirmar con **"Canjear"**.
4. Respuesta visual inmediata: **verde** (canjeado) o **rojo** (ya canjeado / expirado / anulado).

### 61.3 Reglas

- Un cupón se canjea **una sola vez**.
- El partner solo puede canjear los cupones que el administrador le habilitó (o todos, si no tiene restricción).
- Cada canje queda registrado con fecha, comercio y operador para la reportería de beneficios.

## 62. Otras mejoras de la versión 4.0

- **Calendario del portal**: vistas **Gantt / semana / día** más legibles, con auto-scroll a la hora actual.
- **Premiaciones en portales**: las programadas aparecen **primero y destacadas**; las secciones son **colapsables**; deep-link a la premiación exacta desde una notificación.
- **QR de alimentación ampliable** y QR de credencial **siempre visible** en la sección Cuenta.
- **SofIA en el portal VIP**: widget **compacto y arrastrable**, que ya no tapa la pantalla.
- **Idioma del sistema**: la plataforma detecta y aplica el idioma configurado.
- **Acreditaciones**: el listado de conductores incluye también a los **conductores de proveedor**.
- **Beneficios**: la **audiencia de los cupones** usa el tipo de cliente real del participante.
- **Refresh consistente**: al recargar el portal (web o app), el usuario vuelve **al mismo submódulo** en el que estaba.
...
**Un usuario no puede iniciar sesión: "esta cuenta ya tiene una sesión activa en otro dispositivo". ¿Qué hago?**
Es la **sesión única por dispositivo** (capítulo 55). Pedirle que cierre sesión en el otro dispositivo; si la app se cerró sin logout, la sesión caduca sola en ~2 minutos y podrá entrar.

**Un usuario eliminó su cuenta por error. ¿Cómo la recupero?**
Desde el módulo administrativo (capítulo 56.2): la cuenta aparece con badge **ELIMINADA** en su listado correspondiente y el botón **"Reactivar"** restaura el estado previo. El usuario vuelve a entrar con su mismo código.

**¿Eliminar la cuenta borra los datos históricos del participante?**
No. Es una **desactivación permanente de acceso** (soft delete): viajes, canjes y acreditación se conservan para trazabilidad, pero la cuenta no puede volver a ingresar hasta que un administrador la reactive.

**¿Con qué entra un comercio al portal Partner?**
Con su **código de comercio + PIN**, definidos al crearlo en Beneficios → Administrar beneficios. Si olvidó el PIN, el administrador puede definir uno nuevo editando el partner (capítulo 61).
