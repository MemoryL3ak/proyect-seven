  ...
  return (
    <div className="p-4 md:px-7 md:py-6" style={{ maxWidth: 1100, margin: "0 auto" }}>
      <div style={{ marginBottom: 18 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: SURFACE.text, margin: 0 }}>
          Notificaciones push
        </h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(320px,1fr)_1.4fr] gap-4 items-stretch">
        ...
        <div style={{ background: SURFACE.card, border: `1px solid ${SURFACE.border}`, borderRadius: 14, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          ...
          <div style={{ overflowY: "auto", maxHeight: 480 }}>
            ...
            {!loadError && filtered.length === 0 && !loading && (
              <div style={{ padding: 22, fontSize: 12.5, color: SURFACE.textFaint, textAlign: "center" }}>
                {recipients.length === 0
                ...
                >
                  <span
                    style={{
                      ...
                      color: r.userKind === "driver" ? STATE.info : BRAND.tealInk,
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: SURFACE.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                      {display}
                    </p>
                    <p style={{ margin: "2px 0 0", fontSize: 10.5, color: SURFACE.textFaint }}>
                      {r.userKind} · {r.platforms.join(", ")}
                    </p>
                  </div>
                  {isSelected && (
                    <CheckIcon size={16} color={BRAND.teal} strokeWidth={2.5} />
                  )}
                </button>
              );
            })}
        ...
        <div style={{ background: SURFACE.card, border: `1px solid ${SURFACE.border}`, borderRadius: 14, padding: 18 }}>
          <div style={{ marginBottom: 14 }}>
            ...
            {selected && (
              <p style={{ margin: "2px 0 0", fontSize: 11, color: SURFACE.textFaint }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 12, fontWeight: 600, color: SURFACE.textSecondary, display: "block", marginBottom: 4 }}>
              Emoji
            </label>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            />
            <p style={{ margin: "4px 0 0", fontSize: 10.5, color: SURFACE.textFaint, textAlign: "right" }}>
              {body.length}/300
            </p>
          </div>
          ...
          {result?.kind === "ok" && (
            <div
              style={{
                ...
                fontSize: 12, color: BRAND.tealInk,
              }}
            >
