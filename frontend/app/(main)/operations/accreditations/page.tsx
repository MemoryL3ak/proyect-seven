...
export default function AccreditationsPage() {
  ...
  const pal = {
    ...
    cardShadow: "0 1px 4px rgba(15,23,42,0.06)", cardBorder: SURFACE.border,
    innerBg: SURFACE.bg,
    orb1: "rgba(33,208,179,0.15)", orb2: "rgba(31,205,255,0.1)",
  };

  const sel = { background: SURFACE.card, color: SURFACE.text };
  ...
  const generateCredential = async (subjectType: "PARTICIPANT" | "DRIVER", subjectId: string) => {
    ...
    try {
      ...
      const qrDataUrl = await QRCode.toDataURL(scanUrl.toString(), {
        width: 240,
        margin: 1,
        color: { dark: SURFACE.text, light: SURFACE.card },
      });
  ...
  const pctColor = andKpiTotals.pct >= 80 ? STATE.success : andKpiTotals.pct >= 50 ? STATE.warning : STATE.danger;
  const varColor = andKpiTotals.variance < 0 ? STATE.danger : STATE.success;
          ...
          <div style={{ position: "relative" }}>
              ...
              <div>
                <div className="flex items-center gap-2" style={{ marginBottom: "5px" }}>
                  ...
                  <span style={{ display: "inline-flex", alignItems: "center", gap: "5px", background: "rgba(16,185,129,0.15)", border: "1px solid rgba(16,185,129,0.3)", borderRadius: "99px", padding: "2px 10px" }}>
                    <span style={{ width: "6px", height: "6px", borderRadius: "50%", background: STATE.success, display: "inline-block", animation: "pulse 2s ease-in-out infinite" }} />
                    <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.12em", color: STATE.success }}>{t("EN VIVO")}</span>
                  </span>
                </div>
            ...
            <div style={{ background: pal.innerBg, border: `1px solid ${pal.cardBorder}`, borderRadius: "16px", padding: "16px", marginBottom: "18px" }}>
              <div className="grid gap-3 xl:grid-cols-[1.25fr_1.25fr_0.9fr]">
                ...
                <div style={{ background: "rgba(33,208,179,0.08)", border: "1px solid rgba(33,208,179,0.25)", borderRadius: "14px", padding: "14px 16px" }}>
                  <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: BRAND.teal }}>{t("Vista activa")}</p>
            ...
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                { label: "Acreditado",       value: String(andKpiTotals.accredited), color: STATE.success,  sub: "Casos aprobados en acreditación",    extra: null },
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
                ...
                <tbody>
                  {andKpiRows.map((row, idx) => {
                    const rc = row.variance < 0 ? STATE.danger : STATE.success;
                    return (
                      <tr key={row.key} style={{ borderBottom: idx < andKpiRows.length - 1 ? `1px solid ${pal.cardBorder}` : "none" }}>
                        ...
                        <td style={{ padding: "10px 14px", fontWeight: 600, color: STATE.success }}>{row.accredited}</td>
                        ...
                        <td style={{ padding: "10px 14px" }}>
                          <div className="flex items-center gap-2">
                            <div style={{ width: "80px", height: "6px", borderRadius: "99px", background: `${pctColor}22`, flexShrink: 0 }}>
                              <div style={{ height: "6px", borderRadius: "99px", background: row.pct >= 80 ? STATE.success : row.pct >= 50 ? STATE.warning : STATE.danger, width: `${Math.min(row.pct, 100)}%` }} />
                            </div>
                            <span style={{ fontSize: "12px", fontWeight: 700, color: row.pct >= 80 ? STATE.success : row.pct >= 50 ? STATE.warning : STATE.danger }}>{row.pct}%</span>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <section style={{ background: SURFACE.card, border: "1px solid #e2e8f0", borderRadius: "20px", padding: "18px 20px", boxShadow: pal.cardShadow }}>
        ...
        {(error || message) ? (
          <div className="mt-3 space-y-1">
            {error ? <p className="text-sm" style={{ color: STATE.danger }}>{error}</p> : null}
            {message ? <p className="text-sm" style={{ color: STATE.success }}>{message}</p> : null}
          </div>
        ) : null}
      </section>
            ...
            {(selectedAthlete || selectedDriver) ? (
              <div style={{ borderRadius: "14px", border: "1px solid #e2e8f0", background: SURFACE.bg, padding: "14px" }}>
                ...
                <div style={{ marginTop: "14px" }}>
                  <p style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.16em", textTransform: "uppercase", color: SURFACE.textFaint, marginBottom: "8px" }}>{t("Asignacion de accesos")}</p>
                    ...
                    {ACCESS_OPTIONS.map((option) => {
                      ...
                      return (
                        <label key={option.code} style={{
                          ...
                          />
                          <span style={{ display: "inline-flex", minWidth: "32px", alignItems: "center", justifyContent: "center", borderRadius: "8px", background: checked ? "rgba(33,208,179,0.15)" : SURFACE.borderMuted, padding: "2px 6px", fontSize: "11px", fontWeight: 700 }}>{option.code}</span>
        ...
        <div style={{ background: SURFACE.card, border: "1px solid #e2e8f0", borderRadius: "20px", padding: "20px", boxShadow: pal.cardShadow, overflow: "hidden" }} className="xl:order-2">
          {newSubjectType === "PARTICIPANT" ? (
            <>
              ...
              <div style={{ overflowY: "auto", maxHeight: "520px", display: "flex", flexDirection: "column", gap: "8px" }}>
                {participantRows.map((item) => {
                  const acc = accByAthlete[item.id];
                  const accredited = isAccredited(acc?.status);
                  const credIssued = hasCredential(acc);
                  const accessTypes = normalizeAccessTypes(acc?.accessTypes);
                  const photo = photoFromMetadata(item.metadata);
                  const delegCode = item.delegationId ? (delegationMap[item.delegationId]?.countryCode || item.delegationId) : null;
                  return (
                    <div key={item.id} style={{
                      borderRadius: "14px",
                      border: `1px solid ${accredited ? "rgba(16,185,129,0.25)" : "#e2e8f0"}`,
                      borderLeft: `4px solid ${accredited ? "#10b981" : "#cbd5e1"}`,
                      background: accredited ? "rgba(16,185,129,0.03)" : "#fafafa",
                      padding: "12px 14px",
                ...
                {participantRows.length === 0 && (
                  <div style={{ padding: "32px 16px", textAlign: "center", fontSize: "13px", color: SURFACE.textFaint }}>{t("No hay participantes para ese filtro.")}</div>
                )}
              </div>
            </>
          ) : (
            <>
              ...
              <div style={{ overflowY: "auto", maxHeight: "520px", display: "flex", flexDirection: "column", gap: "8px" }}>
                {driverRows.map((item) => {
                  ...
                  const photo = item.photoUrl || photoFromMetadata(item.metadata);
                  const provName = item.providerId ? (providerMap[item.providerId]?.name || null) : null;
                  return (
                    <div key={item.id} style={{
                      borderRadius: "14px",
                      border: `1px solid ${accredited ? "rgba(16,185,129,0.25)" : "#e2e8f0"}`,
                ...
                {driverRows.length === 0 && (
                  <div style={{ padding: "32px 16px", textAlign: "center", fontSize: "13px", color: SURFACE.textFaint }}>{t("No hay conductores para ese filtro.")}</div>
                )}
              </div>
            </>
          )}
