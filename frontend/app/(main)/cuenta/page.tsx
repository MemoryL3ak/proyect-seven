  ...
  return (
    <div className="space-y-4" style={{ maxWidth: 860, animation: "fadeInUp 0.4s ease" }}>
      ...
      <div style={{ ...card, borderTop: `2px solid ${TEAL}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: SURFACE.text, margin: 0 }}>{name}</h2>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
              <span style={{
                fontSize: 11, fontWeight: 800, letterSpacing: "0.08em", textTransform: "uppercase",
                padding: "3px 10px", borderRadius: 99,
                background: "rgba(33,208,179,0.12)", color: BRAND.tealInk, border: "1px solid rgba(33,208,179,0.3)",
              }}>
                {role}
              </span>
              {isUsernameAccount(user?.email) ? (
                ...
                <span style={{ fontSize: 12, color: SURFACE.textMuted, overflow: "hidden", textOverflow: "ellipsis" }}>{user?.email || "—"}</span>
              )}
            </div>
          </div>
        </div>
      ...
      <div style={card}>
        ...
        {!grantedByGroup ? (
          <p style={{ fontSize: 13, color: SURFACE.textMuted, margin: 0 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[...grantedByGroup.entries()].map(([group, mods]) => (
              <div key={group}>
                <p style={{ fontSize: 11, fontWeight: 700, color: SURFACE.textSecondary, margin: "0 0 6px" }}>{group}</p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {mods.map((m) => (
                    <span key={m.id} style={{
                      display: "inline-flex", alignItems: "center", gap: 5, fontSize: 12, fontWeight: 600, padding: "4px 10px", borderRadius: 99,
                      background: "rgba(33,208,179,0.08)", color: BRAND.tealInk, border: "1px solid rgba(33,208,179,0.25)",
                    }}>
            ...
            <p style={{ fontSize: 11.5, color: SURFACE.textFaint, margin: "4px 0 0" }}>
      ...
      <div style={card}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 12, flexWrap: "wrap" }}>
          ...
          {pwMsg && <span style={{ fontSize: 13, color: BRAND.tealInk, fontWeight: 600 }}>{pwMsg}</span>}
          {pwError && <span style={{ fontSize: 13, color: STATE.danger }}>{pwError}</span>}
        </div>
      </div>
      ...
      {!isAdmin && (
        <div style={{ ...card, border: "1px solid rgba(239,68,68,0.3)" }}>
          <p style={{ ...sectionLabel, color: STATE.danger }}>{t("Zona de riesgo")}</p>
          <p style={{ fontSize: 13, color: SURFACE.textMuted, margin: "0 0 12px", lineHeight: 1.5 }}>
          ...
          <button type="button" onClick={() => { setDeleteOpen(true); setDeleteText(""); setDeleteError(null); }}
            style={{
              ...
              background: "rgba(239,68,68,0.08)", color: STATE.danger,
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 18, justifyContent: "flex-end" }}>
              ...
              <button type="button" onClick={handleDelete} disabled={deleting || deleteText.trim().toUpperCase() !== "ELIMINAR"}
                style={{
                  ...
                  background: deleteText.trim().toUpperCase() === "ELIMINAR" && !deleting ? STATE.danger : "rgba(239,68,68,0.35)",
