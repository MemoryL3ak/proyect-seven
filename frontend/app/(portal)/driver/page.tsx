...
type VehicleItem = { id: string; plate?: string | null; type?: string | null };
...
export default function DriverPortalPage() {
  ...
  const uploadPhoto = async (file: File) => {
    ...
    try {
      ...
      const updated = await apiFetch<Driver>(`/drivers/${driverProfile.id}/photo`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <section className="surface rounded-3xl p-6 space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="badge badge-emerald">
            {driverProfile?.status === "INACTIVE" ? t("Inactivo") : t("Activo")}
          </div>
        </div>
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="glass rounded-2xl p-4 md:col-span-2">
            ...
            <p className="text-sm text-slate-600">
              {driverProfile?.providerId
                ? providers[driverProfile.providerId]?.name || "-"
                : "-"}
            </p>
            <p className="text-xs text-slate-500">
              {t("RUT")}:{" "}
              {driverProfile?.providerId
                ? providers[driverProfile.providerId]?.rut || "-"
                : "-"}
            </p>
          </div>
          <div className="glass rounded-2xl p-4 md:col-span-2">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">{t("Foto")}</p>
            <div className="flex flex-wrap items-center gap-4">
              {driverProfile?.photoUrl ? (
                ...
                <div className="h-16 w-16 rounded-full bg-slate-100 flex items-center justify-center text-xs text-slate-400">
                  {t("Sin foto")}
                </div>
              )}
              <label className="btn btn-ghost">
                {photoUploading ? t("Subiendo...") : t("Subir foto")}
                <input
                  type="file"
                  accept="image/*"
                  className="hidden"
              ...
              return (
                <div key={trip.id} className="glass rounded-2xl p-4 space-y-3">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    ...
                    <div className="text-xs text-slate-500">
                      {t("Costo de viaje")}: {formatCurrency(trip.tripCost)}
                    </div>
